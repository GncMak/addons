...
class EgCustomDashboardItem(models.Model):
    ...
    def filter_record(self, date_record_filter_type=None, date_filter_field=None):
        if date_record_filter_type in ['none']:
            ...
            today_datetime = datetime.strptime(fields.Datetime.now(), DEFAULT_SERVER_DATETIME_FORMAT)
            date_filter_list = []
            if date_record_filter_type == 'this_week':
                date_filter_list.append((date_filter_field.name, '>=', str(self.date_start_of(today_datetime, 'week'))))
                date_filter_list.append((date_filter_field.name, '<=', str(self.date_end_of(today_datetime, 'week'))))
            elif date_record_filter_type == 'this_month':
                date_filter_list.append(
                    (date_filter_field.name, '>=', str(self.date_start_of(today_datetime, 'month'))))
                date_filter_list.append((date_filter_field.name, '<=', str(self.date_end_of(today_datetime, 'month'))))
            return date_filter_list
    ...
    def date_start_of(self, value, date_type):
        ...
        if date_type == "month":
            ...
            raise ValueError(
                "date_type must be year, quarter, month, week, day or hour for value %s" % value
            )
        else:
            raise ValueError(
                "date_type must be year, quarter, month, week or day for value %s" % value
            )

        return datetime.combine(result, time.min) if is_datetime else result

    def date_end_of(self, value, date_type):
        is_datetime = isinstance(value, datetime)
        if date_type == "month":
            ...
            raise ValueError(
                "date_type must be month,hour, year,day or week for value %s" % value
            )
        else:
            raise ValueError(
                "date_type must be month,hour, year,day or week for value %s" % value
