...
class CrmSalespersonPlannerVisitTemplate(models.Model):
    ...
    # )
    last_visit_date = fields.Date(
        string="Last Visit Date", store=True
    )
    ...
    def create(self, vals_list):
        name_add = True
        seq = self.env["ir.sequence"].next_by_code("salesperson.planner.visit.template")
        for vals in vals_list:
            if 'name' == vals:
                if vals == "/":
                    ...
                    name_add = False
        if name_add:
            vals_list.update({"name": seq})
        res = super(CrmSalespersonPlannerVisitTemplate, self).create(vals_list)
        return res
    ...
    def filter_dates(self, recurrences, days=7):
        ...
        max_date = datetime.strptime(fields.Date.today(), '%Y-%m-%d') + relativedelta(
